...
case class FETA(s:FSystem,st:FSTs):
  ...
  val products = fm.products(this.features)
  ...
  //  cas.forall(ca=> fca(ca).enabledOut(gs.states(ca)).isEmpty)
  ...
  protected def mkActRsp(a:CAction, comb:Set[Set[CName]],q:SysSt):FReq =
    val rsps = comb.map(cas=> FRsp(cas,a,feReq(cas,a,q)&&inFe(cas,q)))
  ...
  protected def mkRsp(a:CAction,participants:Set[CName],q:SysSt):Option[FRsp] =
    ...
    val prods = st.satisfiedBy(req)
    if prods.isEmpty then None else Some(FRsp(req.at,req.act,req.fe && fe(prods) && inFe(participants,q)))
