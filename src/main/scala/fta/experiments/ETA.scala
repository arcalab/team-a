...
import fta.experiments.ETA.{SType, StReq}
import fta.experiments.System.{CName, SysLabel, SysSt, SysTrans}
import fta.experiments.CA.CAction
import fta.experiments.ETA.ST


case class ETA(s:System,st:ST) extends LTS[SysSt,SysLabel]: 
  type Trans = SysTrans
  
  val states:Set[SysSt] = reachableSt
  
  val initial:Set[SysSt] = s.initial
  
  val labels:Set[SysLabel] = s.labels
  
  val trans:Set[SysTrans] = reachableTrans

  protected val allTrans:Set[SysTrans] = s.trans.filter(t=>
    if s.communicating.contains(t.by.action) then (st(t.by.action).satisfies(t.by)) else true)

  protected val (reachableSt,reachableTrans):(Set[SysSt],Set[SysTrans]) = reachable(allTrans)
